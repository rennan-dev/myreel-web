...
import { IconPlus, IconCheck, IconPlay, IconCalendar } from '../ui/icons';
 ...
 */
export function SeasonsSection({ media, onChanged }) {
    ...
    return (
        <div>
            ...
            <div className="flex flex-col gap-4">
                ...
                {seasons.length === 0 && (
                    <p className="rounded-xl border border-dashed border-white/12 bg-white/[0.02] px-4 py-8 text-center text-sm text-muted-foreground">Nenhuma temporada cadastrada ainda. Informe o número acima para gerar.</p>
                )}
            </div>
        </div>
    );
...
function SeasonCard({ season, busy, episodeCountValue, onDateChange, onSaveDate, onToggleEpisode, onEpisodeCountChange, onBulkEpisodes }) {
    ...
    return (
        <Card className="p-4">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
                {sortedEps.map((ep) => (
                    <button key={ep.id} type="button" disabled={Boolean(busy[`ep-${ep.id}`])} onClick={() => onToggleEpisode(ep)} title={ep.is_watched ? 'Clique para desmarcar' : 'Clique para marcar como assistido'}
